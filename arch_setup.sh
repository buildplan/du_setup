...
print_header() {
    ...
    printf '%s\n' "${CYAN}╔═════════════════════════════════════════════════════════════════╗${NC}"
    printf '%s\n' "${CYAN}║                                                                 ║${NC}"
    printf '%s\n' "${CYAN}║          ARCH LINUX SERVER SETUP AND HARDENING SCRIPT           ║${NC}"
    printf '%s\n' "${CYAN}║                   v0.81.4-arch.1 | 2026-09-08                   ║${NC}"
    printf '%s\n' "${CYAN}║                                                                 ║${NC}"
    printf '%s\n' "${CYAN}╚═════════════════════════════════════════════════════════════════╝${NC}"
    printf '\n'
}
...
configure_ssh() {
    ...
    if [[ "$SSH_SERVICE" == "ssh.socket" ]]; then
        ...
        printf '%s\n' "[Service]" "ExecStart=" "ExecStart=/usr/bin/sshd -D -p $SSH_PORT" > "/etc/systemd/system/${SSH_SERVICE}.d/override.conf"
    fi
...
rollback_ssh_changes() {
    ...
    if systemctl list-units --full -all --no-pager | grep -E "[[:space:]]ssh.socket[[:space:]]" >/dev/null 2>&1; then
        ...
        SSH_SERVICE="sshd.service" # Fallback for Arch
        print_warning "SSH service '$initial_service_check' not found, falling back to '$SSH_SERVICE'."
        log "Rollback warning: Using fallback SSH service sshd.service."
    ...
    if [[ -f "$SSHD_BACKUP_FILE" ]]; then
        ...
        if [[ "$USE_SOCKET" == true ]]; then
            ...
            local service_for_rollback="sshd.service"
            ...
            printf '%s\n' "[Service]" "ExecStart=" "ExecStart=/usr/bin/sshd -D -p $PREVIOUS_SSH_PORT" > "/etc/systemd/system/${service_for_rollback}.d/override.conf"
        fi
    else
    ...
    if ! /usr/bin/sshd -t >/tmp/sshd_config_test.log 2>&1; then
        ...
        print_info "Action: Fix /etc/ssh/sshd_config manually and test with 'sshd -t', then restart with 'systemctl restart sshd.service'."
        return 0
    fi
    ...
    if [[ "$USE_SOCKET" == true ]]; then
        ...
        if ! systemctl restart ssh.service 2>/tmp/sshd_restart.log; then
            ...
            timeout 5 /usr/bin/sshd -D -f /etc/ssh/sshd_config >/tmp/sshd_manual_start.log 2>&1
            local TIMEOUT_EXIT=$?
...
generate_summary() {
    ...
    for service in "$SSH_SERVICE" chronyd; do
...
main() {
    ...
    log "Starting Arch Linux hardening script."
