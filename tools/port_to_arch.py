...
# ---------------------------------------------------------------- fixes found while testing in a VM
# openbsd-netcat conflicts with gnu-netcat and pacman --noconfirm refuses to replace it
sub('''    print_info "Installing essential packages..."
    if ! pacman -S --needed --noconfirm \\
        ufw chrony rsync wget curl \\
        vim htop iotop nethogs openbsd-netcat ncdu \\
        tree cronie jq gawk coreutils perl skopeo git \\
...
''', '''    print_info "Installing essential packages..."
    local -a ESSENTIAL_PKGS=(
        ufw chrony rsync wget curl
        vim htop iotop nethogs ncdu
        tree cronie jq gawk coreutils perl skopeo git
        ca-certificates gnupg logrotate make inetutils
        pacman-contrib arch-audit dmidecode openssh
    )
    # openbsd-netcat conflicts with gnu-netcat; only add it when no 'nc' is present
    command -v nc >/dev/null 2>&1 || ESSENTIAL_PKGS+=(openbsd-netcat)
    if ! pacman -S --needed --noconfirm "${ESSENTIAL_PKGS[@]}"; then
''')
# do not back up a .bashrc that is just the untouched /etc/skel copy
sub('''    if [[ -f "$BASHRC_PATH" ]] && ! grep -q "generated by /usr/sbin/adduser" "$BASHRC_PATH" 2>/dev/null; then''',
    '''    if [[ -f "$BASHRC_PATH" ]] && ! grep -q "generated by /usr/sbin/adduser" "$BASHRC_PATH" 2>/dev/null && ! cmp -s "$BASHRC_PATH" /etc/skel/.bashrc; then''')
# rollback hints should name the Arch service
sub('''then restart with 'systemctl restart ssh.service'."''', '''then restart with 'systemctl restart sshd.service'."''')
sub('''print_info "Try starting SSH with 'sudo systemctl start ssh.service'."''',
    '''print_info "Try starting SSH with 'sudo systemctl start sshd.service'."''')
# Secure DNS: verify resolution actually works afterwards; restrictive networks block external
# resolvers (53/853) and the whole rest of the run (pacman, Docker, Lynis) would fail silently.
sub('''    print_success "Secure DNS configured and activated."
...
''')
# arch-audit needs network access; report "Unknown" instead of a misleading 0 when it fails
sub('''            print_info "Running arch-audit..."
            {
                printf '\\n=== arch-audit (%s) ===\\n' "$(date)"
                arch-audit
            } >> "$AUDIT_LOG" 2>&1 || true
